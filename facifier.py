    ...
    while read_value:
            ...
            for normalized_face, (x, y, w, h) in find_faces(webcam_image):
                ...
                for (i,rect) in enumerate(rects):
                    ...
                    output = face_utils.visualize_facial_landmarks(webcam_image, shape)
                    #if init or delay == 0:
                    #init = False
                    ...
                    if (gender_prediction[0] == 0):
                      ...
                      cv2.putText(webcam_image, "Gender predictor: Female", (x,y-50),cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255), 2)
                    else:
                      cv2.rectangle(webcam_image, (x,y), (x+w, y+h), (255,0,0), 2)
                      cv2.imshow('video', output)
                      cv2.putText(webcam_image, "Gender predictor: Male", (x,y-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255), 2)
...
def analyze_picture(model_emotion, model_gender, path, window_size, window_name='static'):
    ...
    for normalized_face, (x, y, w, h) in find_faces(image):
        ...
        if (gender_prediction[0] == 0):
            ...
            cv2.putText(image, "Gender predictor: Female", (x,y-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255), 2)
        else:
            cv2.rectangle(image, (x,y), (x+w, y+h), (255,0,0), 2)
            cv2.putText(image, "Gender predictor: Male", (x,y-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255), 2)
...
if __name__ == '__main__':
    ...
    if (choice == 'y'):
        ...
        print("Default path is set to img/")
